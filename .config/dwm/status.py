...
def get_temperature_from_sensors():
    try:
        ...
        # You can adjust this list based on your sensor names
        for key in ['Composite', 'Package id0']:
...
def get_weather_icon(keyword):
    icon_map = {
        ...
        'blizzard': '\uf064',
        'fog': '\uf014',
        # Using moon for freezing conditions (no direct match)
        'freezing': '\uf076',
...
def print_statusbar():
    ...
    while True:
        ...
        if cpu_temp is not None:
            set_dwm_status(
                f"  \uf092  { current_time_str} | \uf2db {cpu_temp}°C | {weather_info}")
